...
class NN:
    ...
    def __init(self):
        ...
        self.__program_start_time = self.TIME  # 程序的开始时间
    ...
    @staticmethod
    def init_weight(shape, name='weight'):
    ...
    def restore_model(self):
        self.saver.restore(self.sess, self.get_model_path())
    ...
    def get_model_path(self):
        ...
        model_dir = os.path.join(cur_dir, 'saved_models')
    ...
    def __run_tensorboard_sync(path, port=6006):
        ...
        try:
            env = 'tensor' if '2.7' in sys.version else 'python35'  # python 所在的 conda 环境
            prefix = 'source' if os_system != 'windows' else ''  # 判断是什么操作系统
            split_or = ';' if os_system != 'windows' else '&'

            # 若无需激活 python 环境，可把前面的 activate 相关语句删除
            NN.cmd('%s activate %s%s tensorboard --logdir=%s --port=%d' % (prefix, env, split_or, path, port))
    ...
    def parse_model(self, X):
        ...
        for i, config in enumerate(self.MODEL):
            ...
            # 池化层
                ...
                with tf.name_scope(name):
                    if isinstance(config['k_size'], list):
    ...
    def conv2d(x, W, stride=1, padding='SAME'):
        if len(x.shape) == 3:
            return NN.conv1d(x, W, stride, padding)
    ...
    def conv2d_bias(x, W, b, stride=1, padding='SAME'):
        if len(x.shape) == 3:
            conv = NN.conv1d(x, W, stride, padding)
        else:
    ...
    @staticmethod
    def max_pool(x, k_size, strides=None):
